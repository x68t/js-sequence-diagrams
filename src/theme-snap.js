...
if (typeof Snap != 'undefined') {
  ...
  _.extend(SnapTheme.prototype, BaseTheme.prototype, {
    ...
    setupPaper: function(container) {
      ...
      var a = this.arrowMarkers_ = {left: {}, right: {}};
      var arrow = this.paper_.path('M 0 0 L 5 2.5 L 0 5 z');
      a.right[ARROWTYPE.FILLED] = arrow.marker(0, 0, 5, 5, 5, 2.5)
       .attr({id: 'markerRightArrowBlock'});
      arrow = arrow.clone();
      arrow.transform('s -1 1');
      a.left[ARROWTYPE.FILLED] = arrow.marker(0, 0, 5, 5, 0, 2.5)
        .attr({id: 'markerLeftArrowBlock'});

      arrow = this.paper_.path('M 9.6,8 1.92,16 0,13.7 5.76,8 0,2.286 1.92,0 9.6,8 z');
      a.right[ARROWTYPE.OPEN] = arrow.marker(0, 0, 9.6, 16, 9.6, 8)
        .attr({markerWidth: '4', id: 'markerRightArrowOpen'});
      arrow = arrow.clone();
      arrow.transform('s -1 1');
      a.left[ARROWTYPE.OPEN] = arrow.marker(0, 0, 9.6, 16, 0, 8)
        .attr({markerWidth: '4', id: 'markerLeftArrowOpen'});

      a.right[ARROWTYPE.NONE] = undefined;
      a.left[ARROWTYPE.NONE] = undefined;
    },
    ...
    drawLine: function(x1, y1, x2, y2, linetype, leftArrowhead, rightArrowhead) {
        var line = this.paper_.line(x1, y1, x2, y2).attr(LINE);
        if (linetype !== undefined) {
          line.attr('strokeDasharray', this.lineTypes_[linetype]);
        }
        var arrowhead = leftArrowhead;
        if (arrowhead !== undefined) {
          line.attr('markerStart', this.arrowMarkers_.left[arrowhead]);
        }
        arrowhead = rightArrowhead;
        if (arrowhead !== undefined) {
          line.attr('markerEnd', this.arrowMarkers_.right[arrowhead]);
        }
